...
- Ein Azure-Abonnement, auf das Sie mindestens als Mitwirkender zugreifen können. Falls Sie nicht über ein Azure-Abonnement verfügen, können Sie sich für eine [kostenlose Testversion](https://azure.microsoft.com/free) registrieren.
...
1. Wechseln Sie zum Browserfenster, in dem Ihr GitHub-Konto angezeigt wird, und stellen Sie sicher, dass Sie weiterhin authentifiziert sind (falls nicht, melden Sie sich mit Ihrem GitHub-Benutzerkonto an).
...
1. Öffnen Sie das Verzeichnis **.github/workflows**, und wählen Sie dann die Datei **eshoponweb-cicd.yml** aus, um deren Inhalt anzuzeigen. Beachten Sie, dass dieser GitHub Actions-Workflow die Aufträge **buildandtest** und **deploy** enthält, die aus den folgenden Schritten bestehen:
...
1. Beachten Sie, dass sich der Workflow auf ein Geheimnis verlässt, um sich während des Schritts „Einrichten der Azure CLI“ beim Ziel-Azure-Abonnement zu authentifizieren, wie die Anweisung `creds: ${{ secrets.AZURE_CREDENTIALS }}` zeigt.
...
1. Geben Sie im Azure-Portal oben auf der Seite im Suchtextfeld **Ressourcengruppen** ein, und wählen Sie in der Ergebnisliste **Ressourcengruppen** aus.
1. Wählen Sie auf der Seite **Ressourcengruppen** **+ Erstellen** aus.
1. Geben Sie in das Textfeld **Ressourcengruppen** **rg-eshoponweb-westeurope** ein.
...
1. Wählen Sie auf der Seite **Ressourcengruppen** **+ Erstellen** aus.
1. Geben Sie in das Textfeld **Ressourcengruppen** **rg-eshoponweb-eastus** ein.
...
1. Kopieren Sie den Wert der Abonnement-ID, die vom zweiten Befehl zurückgegeben wird, und notieren Sie ihn. Er wird später in dieser Übung benötigt.
...
1. Kopieren Sie die gesamte JSON-formatierte Ausgabe des Befehls, und notieren Sie sie. Sie benötigen ihn später. Das Format der Ausgabe sollte dem des folgenden Textes ähneln:
...
1. Geben Sie im Bereich **Aktionsgeheimnisse / Neues Geheimnis** in das Textfeld **Name** den Wert **AZURE_CREDENTIALS** ein.
...
1. Wechseln Sie zurück zum Webbrowser, in dem im Cloud Shell-Bereich die Bash-Sitzung angezeigt wird, und führen Sie den folgenden Befehl aus, um den Namen der ersten App Service-Web-App zu generieren, die Sie bereitstellen:

   ```cli
   echo devops-webapp-westeurope-$RANDOM$RANDOM
   ```

1. Kopieren Sie den vom Befehl zurückgegebenen Wert, und notieren Sie ihn. Er wird später in dieser Übung benötigt.
1. Führen Sie in der Bash-Sitzung im Cloud Shell-Bereich den folgenden Befehl aus, um den Namen der zweiten App Service-Web-App zu generieren, die Sie bereitstellen:
...
1. Wenn Sie aufgefordert werden, GitHub Actions zu aktivieren, wählen Sie **Ich kenne meine Workflows; fortfahren und aktivieren**.
...
1. Geben Sie im Webbrowserfenster, in dem das Azure-Portal angezeigt wird, oben auf der Seite im Suchtextfeld **App Services** ein, und wählen Sie in der Ergebnisliste **App Services** aus.
...
1. Überprüfen Sie auf der neuen Browserregisterkarte, ob die Web-App angezeigt wird und ob sie funktioniert. Sie können die zweite Web-App in der Region **USA, Osten** auf die gleiche Weise überprüfen.
